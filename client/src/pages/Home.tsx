...
export default function Home() {
  ...
  // Dynamically create a search keyword based on the first ingredient entered
  const primaryIngredient = ingredients.split(",")[0].trim() || "food";
  const dynamicImageUrl = `https://unsplash.com{encodeURIComponent(primaryIngredient)}`;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-emerald-100 text-gray-800 flex flex-col items-center justify-center p-6">
      <div className="max-w-2xl w-full bg-white rounded-2xl shadow-xl p-8 border border-emerald-100">
        ...
        {/* 🍽️ Output Panel & Dynamic Final Product Image */}
        {recipe && (
          <div className="mt-8 p-6 bg-emerald-50 rounded-xl border border-emerald-200">
            ...
            {/* Dynamic Visual Media Box */}
            <div className="w-full h-64 overflow-hidden rounded-xl mb-6 shadow-md border border-emerald-100">
              <img 
                src={dynamicImageUrl} 
                alt={`A delicious dish made with ${primaryIngredient}`} 
