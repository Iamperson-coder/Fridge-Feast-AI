                                                                ...
                                                                export default function Home() {
                                                                  ...
                                                                  const handleGenerate = () => {
                                                                    ...
                                                                    setTimeout(() => {
                                                                      setRecipe({
                                                                        ...
                                                                        description: "A delicious customized culinary creation showcasing your fresh combination of ingredients.",
                                                                        prepTime: "10 mins",
                                                                        cookTime: "15 mins",
                                                                        servings: 2,
                                                                        ingredients: ingredients.map(name => ({ name, amount: getMeasurement(name) })),
                                                                        instructions: [
                                                                          "Preheat your cooking pan over a medium flame and add your designated cooking oil.",
                                                                          "Gently organize, slice, and prepare your items according to the specific measurements listed.",
                                                                  ...
                                                                  return (
                                                                    <div className="min-h-screen bg-[#faf8f5] text-slate-800 p-4 md:p-8">
                                                                      <header className="max-w-6xl mx-auto text-center mb-12 pt-6">
                                                                        <h1 className="text-4xl md:text-5xl font-bold text-slate-900 mb-2">⚔️ THE AI KITCHEN ⚔️</h1>
                                                                        <p className="text-slate-500 italic text-lg">"Culinary magic from your ingredients"</p>
                                                                      </header>
                                                                      ...
                                                                      <main className="max-w-6xl mx-auto grid lg:grid-cols-5 gap-8 lg:gap-12">
                                                                          ...
                                                                          <div className="bg-white p-6 md:p-8 rounded-sm shadow-sm border-t-4 border-[#8b263e] relative overflow-hidden">
                                                                            <AnimatePresence>
                                                                              {isScanning && (
                                                                                <div className="absolute inset-0 bg-white/95 z-50 flex flex-col items-center justify-center text-center p-6">
                                                                                  ...
                                                                                  <h4 className="font-bold text-xl text-[#8b263e]">AI Vision Scanner</h4>
                                                                            ...
                                                                            <div className="flex items-center justify-between mb-6">
                                                                              <div className="flex items-center gap-3">
                                                                                ...
                                                                                <div>
                                                                                  <h2 className="text-2xl font-bold text-slate-900">Chef's Request</h2>
                                                                            ...
                                                                            <div className="mt-8 flex gap-4">
                                                                              <button onClick={handleGenerate} disabled={ingredients.length === 0 || isPending || isScanning} className="flex-1 py-3 px-6 rounded-md font-bold text-lg bg-[#8b263e] text-white shadow-lg shadow-red-900/10 hover:bg-[#721f33] transition-all flex items-center justify-center gap-2 disabled:opacity-40 disabled:cursor-not-allowed">
                                                                                {isPending ? <><Loader2 className="w-5 h-5 animate-spin" /><span>Cooking...</span></> : <><Sparkles className="w-5 h-5" /><span>Create Menu</span></>}
                                                                              </button>
                                                                              {ingredients.length > 0 && (
                                                                                <button onClick={() => { setIngredients([]); setRecipe(null); }} disabled={isPending || isScanning} className="px-4 py-3 rounded-md border border-slate-200 text-slate-700 hover:bg-slate-50">Clear</button>
                                                                              )}
                                                                            </div>
                                                                          </div>
                                                                        </section>
                                                                        ...
                                                                        <section className="lg:col-span-3 min-h-[500px] flex flex-col">
                                                                          <AnimatePresence mode="wait">
                                                                            {isPending ? (
                                                                              <div className="flex-1 flex flex-col items-center justify-center text-center p-12 bg-white border border-slate-100 rounded-sm shadow-sm">
                                                                                <ChefHat className="w-24 h-24 text-[#8b263e] mb-4 animate-bounce" />
                                                                                <h3 className="text-2xl font-bold text-slate-900 mb-2">The Kitchen is Busy</h3>
                                                                                <p className="text-slate-500 italic">Our AI chefs are crafting your custom menu recipe...</p>
                                                                              </div>
                                                                            ) : recipe ? (
                                                                              <div className="bg-white p-6 md:p-8 rounded-sm shadow-sm border border-slate-100">
                                                                                <div className="flex flex-wrap items-center justify-between gap-2 border-b pb-4 mb-4">
                                                                                  <div>
                                                                                    <span className="text-xs uppercase tracking-wider text-slate-400 font-semibold">{recipe.title}</span>
                                                                                    <h2 className="text-3xl font-bold text-slate-900 mt-0.5">{recipe.dishName}</h2>
                                                                                  </div>
                                                                                  <div className="flex items-center gap-1.5 px-3 py-1 bg-slate-100 rounded-full text-slate-600 text-sm font-medium border shadow-sm">
                                                                                    <span>Origin: {recipe.originCountry}</span>
                                                                                  </div>
                                                                                </div>

                                                                                <p className="text-slate-600 italic mb-6">{recipe.description}</p>

                                                                                <div className="flex gap-6 mb-6 text-sm bg-slate-50 p-3 rounded-md border text-slate-600">
                                                                                ...
                                                                                <div className="mb-6">
                                                                                  <h4 className="font-bold text-lg mb-2 border-b pb-1 text-[#8b263e]">Measurements Plotted:</h4>
                                                                                  <ul className="space-y-1.5 text-slate-700 text-sm italic">
                                                                                    {recipe.ingredients.map((ing, i) => (
                                                                                      <li key={i} className="flex justify-between max-w-xs border-b border-dashed border-slate-100 pb-1">
                                                                                ...
                                                                                <div>
                                                                                  <h4 className="font-bold text-lg mb-2 border-b pb-1 text-[#8b263e]">Instructions:</h4>
